...
/*added by psec*/
uint64_t intel_driver::MmAllocatePagesForMdl(HANDLE device_handle, LARGE_INTEGER LowAddress, LARGE_INTEGER HighAddress, LARGE_INTEGER SkipBytes, SIZE_T TotalBytes, MEMORY_CACHING_TYPE CacheType, ULONG Flags)
{
	...
	if (!CallKernelFunction(device_handle, &allocated_pages, kernel_MmAllocatePagesForMdl, LowAddress, HighAddress, SkipBytes, TotalBytes, CacheType, Flags))
		return 0;

	return allocated_pages;
}

uint64_t intel_driver::MmMapLockedPagesSpecifyCache(HANDLE device_handle, uint64_t pmdl, KPROCESSOR_MODE AccessMode, MEMORY_CACHING_TYPE CacheType, uint64_t RequestedAddress, ULONG BugCheckOnFailure, ULONG Priority)
{
...
bool intel_driver::MmUnlockPages(HANDLE device_handle, uint64_t MemoryDescriptorList)
{
	...
	if (!kernel_MmUnlockPages)
	{
		Log(L"[!] Failed to find MmUnlockPages" << std::endl);
		return 0;
	}
...
{
	static uint64_t kernel_IoFreeMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, "IoFreeMdl");

	if (!kernel_IoFreeMdl)
	{
		Log(L"[!] Failed to find IoFreeMdl" << std::endl);
		return 0;
	}

	void* result;
	return CallKernelFunction(device_handle, &result, kernel_IoFreeMdl, Mdl);
}
